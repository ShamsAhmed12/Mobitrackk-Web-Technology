...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action']) && $_POST['action']==='profile') {
    ...
    elseif($conn===null) $generalErr=($db_error??"Database not available.");
        ...
        if($chk){ $chk->bind_param("si",$n_email,$uid); $chk->execute(); $chk->store_result(); if($chk->num_rows>0){ $generalErr="Email already used by another account."; $chk->close(); } else { $chk->close();
    ...
    elseif($conn===null) $generalErr=($db_error??"Database not available.");
        ...
        if(!$stmt) $generalErr="DB error: ".e($conn->error);
            ...
            if(!$row || !password_verify($cur,$row['password'])) $generalErr="Current password is incorrect.";
                ...
                if(!$stmt) $generalErr="DB error: ".e($conn->error);
                else { $stmt->bind_param("si",$hash,$uid); if($stmt->execute()) $successMsg="Password changed."; else $generalErr="Update failed: ".e($stmt->error); $stmt->close(); }
            }
        }
    }
}
...
<div class="card" style="max-width:650px; margin:0 auto;">
    <div class="card-header"><h1>Admin Profile</h1><p>Update your basic information.</p></div>
